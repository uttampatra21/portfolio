        ...
        >
          ...
          <div className="grid md:grid-cols-2 gap-8">
            {education.map((edu, index) => (
              <motion.div
                key={edu.degree}
                className="bg-white dark:bg-gray-800 p-6 rounded-2xl shadow-lg border border-slate-100 dark:border-slate-700 hover:shadow-xl transition-all duration-300 flex flex-col justify-between"
                initial={{ opacity: 0, y: 30 }}
                whileInView={{ opacity: 1, y: 0 }}
                transition={{ duration: 0.5, delay: index * 0.1 }}
                viewport={{ once: true }}
              ...
              >
                <div>
                  <div className="flex items-start justify-between mb-4">
                    <div>
                      <h3 className="font-bold text-lg text-gray-900 dark:text-white leading-snug">{edu.degree}</h3>
                      <p className="text-sm text-purple-600 dark:text-purple-400 font-medium mt-1">{edu.institution}</p>
                    </div>
                    <span className="text-xs font-semibold px-2.5 py-1 bg-purple-50 dark:bg-purple-900/30 text-purple-600 dark:text-purple-400 rounded-full shrink-0 ml-2">
                      {edu.period}
                    </span>
                  </div>
                  <p className="text-sm text-gray-600 dark:text-gray-300 leading-relaxed">{edu.details}</p>
                </div>
              </motion.div>
            ))}
          </div>
              ...
              >
                <div>
                  <div className="flex items-center gap-4 mb-4">
                    {cert.logoType === "hackerrank" ? (
                      ...
                      <div className="w-12 h-12 bg-purple-500/10 dark:bg-purple-500/20 rounded-xl flex items-center justify-center text-purple-600 dark:text-purple-400 font-bold text-xs">
                        WASS
                      </div>
                    )}
                    <div>
                      <h3 className="font-bold text-lg text-gray-900 dark:text-white leading-snug">{cert.title}</h3>
                      <p className="text-sm text-purple-600 dark:text-purple-400 font-medium">{cert.issuer}</p>
                    </div>
                  </div>

                  <div className="space-y-2 mb-6">
                    <p className="text-xs text-gray-400 dark:text-gray-500">{cert.period}</p>
                    {cert.description && (
                      <p className="text-sm text-gray-655 dark:text-gray-350 leading-relaxed mb-3">{cert.description}</p>
                    )}
                    <div className="flex items-center gap-1.5 text-xs font-semibold text-gray-500 dark:text-gray-400 bg-gray-100 dark:bg-gray-700/50 px-2.5 py-1 rounded-md w-fit">
                ...
                {cert.credentialUrl !== "#" ? (
                  <Button
                    variant="outline"
                    size="sm"
                    className="w-full mt-2 border-purple-200 hover:border-purple-300 dark:border-slate-700 text-purple-600 dark:text-purple-400"
                    asChild
                  >
                  ...
                  <div className="text-xs text-center text-gray-400 dark:text-gray-550 border border-dashed border-gray-250 dark:border-slate-750 py-2 rounded-lg mt-2">
